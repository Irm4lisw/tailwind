...
const Beranda = () => {
  ...
  const Pencarian = cari.get("Pencarian"); //punya pak fajri cariProduct
  ...
  // const search = (Input) => {setCari({Pencarian: Input});};
  ...
  const ubahCari = useCallback(
    async (input) => {
      ...
      const response = await axios.get(
        "https://restaurant-api.dicoding.dev/search?q=" + Pencarian
      );
      const data = await response.data;
      // setHasilCari(data);
  ...
  //       <input type="text" className="grow cari" placeholder="Search" onChange={(input)=>ubahCari(input.target.value)} />
  //       <svg
  ...
  //       </svg>
  //     </label>
  //     <p className="mt-3">Pencarian anda : {Pencarian}, ditemukan : {hasilCari?.founded}</p>
  ...
  //               <Link to={"/detail/" + data.id} className="btn btn-primary mt-2">
  //                 Show
  //               </Link>
  //             </div>
  //           </div>
